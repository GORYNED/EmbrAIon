...
`routing.overrides.<host>.routes`

and, when a role-specific choice is needed:

`routing.overrides.<host>.roles`
...
5. Edit only the relevant `routing.overrides` subtree in `.embraion/project.yaml`; preserve unrelated project policy and configuration.
...
- Do not rewrite unrelated sections of `.embraion/project.yaml`.
