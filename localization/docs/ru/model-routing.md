...
Если проект не задаёт override, выбранный AI-клиент использует собственный default/automatic выбор модели. При необходимости проект может сохранить произвольные host-specific значения `model`, `effort` и `options` в `.embraion/routing.yaml` → `overrides`.
