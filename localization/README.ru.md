...
`embraion validate` проверяет данные системы, входящие в активную установку EmbrAIon. `embraion doctor` дополнительно анализирует текущий проект или Git worktree, поэтому запускайте его из нужного репозитория либо из пустой тестовой папки для проверки самой установки.
...
### Что сейчас делают `init`, `install` и `sync`

- `embraion init` создаёт локальный для репозитория слой `.embraion/project.yaml`.
- `embraion install` генерирует одно представление клиента из данных системы, доступных **текущему запущенному CLI**, и копирует его в указанную папку.
- `embraion sync` генерирует одноразовые представления в указанную папку; команда не ищет, не инициализирует и не подключает автоматически все проекты на компьютере.

### Фиксация версии проекта в v0.1.x

`.embraion/project.yaml` записывает версию системы, но текущий CLI `v0.1.x` **не выбирает и не запускает эту версию автоматически**. Команды `install` и `sync` используют данные системы, входящие в тот исполняемый EmbrAIon, который запущен сейчас.

Для точной воспроизводимости используйте версию CLI/системы, совпадающую с версией, записанной в проекте. Поэтому в `v0.1.x` поле версии проекта является границей совместимости и обновления, но пока не является автоматическим механизмом выбора runtime-версии для каждого проекта.
