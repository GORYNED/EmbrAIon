...
Projection ownership state under `.embraion/state/` is intentionally local and gitignored. When a framework update runs in a fresh worktree with no local projection ledger, EmbrAIon attempts a conservative ownership recovery from the exact previous project pin. A file is recovered only when its current bytes exactly match the projection generated by that previous published EmbrAIon version. The recovered evidence is temporary and is promoted into ordinary local projection state after a successful install.

If the previous runtime cannot be resolved, the destination differs, or a file has been edited, EmbrAIon does not infer ownership and keeps reporting a conflict. Recovery never behaves like `--force`.
